/* * KynexOs v142.0 - The Absolute Final Master (Flash & Flicker Fix)
 * Geliştirici: Muhammed (Kynex)
 * Donanım: ESP32-S3 N16R8 (DIO+OPI Hybrid)
 * Özellikler: Dual-Boot RetroGo, NTP Clock, About Links, WiFi Master, Paint, Snake
 * Hata Düzeltme: Partition Table Overflow Fix, LEDC Dimming Safety, Boot Loop Protection
...
void drawDesktop(int hIdx) {
    ...
    if (wallpaper_jpg_start != nullptr) {
        ...
        if (wlen > 100) TJpgDec.drawJpg(0, 0, wallpaper_jpg_start, wlen); 
    }
...
void drawAboutScreen() {
    ...
    tft.setCursor(10, 80); tft.print("Surum: v142.0 Fix");
...
void setup() {
    // 1. ÖNCELİK: EKRAN AYDINLATMASI VE DONANIMI UYANDIR
    pinMode(TFT_BL, OUTPUT);
    digitalWrite(TFT_BL, HIGH); // Flicker engellemek için hemen yak
...
void loop() {
    ...
    if (j1.btn) {
        ...
        if (btnPressStart != 0 && !longPressTriggered) { 
    ...
    if (ts.touched() && millis() - lastTouchTime > 400) {
        ...
        handleGlobalClick(map(p.x, 200, 3850, 320, 0), map(p.y, 240, 3800, 240, 0)); 
